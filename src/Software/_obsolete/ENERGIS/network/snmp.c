...
void snmpd_init(uint8_t *managerIP, uint8_t *agentIP, uint8_t sn_agent, uint8_t sn_trap) {
    ...
    /*
        ...
        {
                    dataEntryType enterprise_oid = {0x0a, {0x2b, 0x06, 0x01, 0x04, 0x01, 0x81, 0x9b,
       0x19, 0x01, 0x00}, SNMPDTYPE_OBJ_ID, 0x0a, {"\x2b\x06\x01\x04\x01\x81\x9b\x19\x10\x00"},
       NULL, NULL};

                    dataEntryType trap_oid1 = {8, {0x2b, 6, 1, 4, 1, 0, 11, 0},
       SNMPDTYPE_OCTET_STRING, 30, {""}, NULL, NULL}; dataEntryType trap_oid2 = {8, {0x2b, 6, 1, 4,
       1, 0, 12, 0}, SNMPDTYPE_INTEGER, 4, {""}, NULL, NULL};
                    ...
                    snmp_sendTrap((void *)"192.168.0.214", (void *)"192.168.0.112", (void
       *)"public", enterprise_oid, SNMPTRAP_WARMSTART, 0, 0);

                    // Enterprise-Specific Trap
                    snmp_sendTrap((void *)"192.168.0.214", (void *)"192.168.0.112", (void
       *)"public", enterprise_oid, 6, 0, 2, &trap_oid1, &trap_oid2);
            }
    */
}
...
int32_t snmpd_run(void) {
    ...
    case SOCK_UDP:
        if ((len = getSn_RX_RSR(SOCK_SNMP_AGENT)) > 0) {
            request_msg.len =
                recvfrom(SOCK_SNMP_AGENT, request_msg.buffer, len, svr_addr, &svr_port);
        } else {
            request_msg.len = 0;
        }
    ...
    case SOCK_CLOSED:
        if ((ret = socket(SOCK_SNMP_AGENT, Sn_MR_UDP, PORT_SNMP_AGENT, 0x00)) != SOCK_SNMP_AGENT)
            return ret;
#ifdef _SNMP_DEBUG_
        printf("[INFO] - [%d] UDP Socket for SNMP Agent, port [%d]\r\n", SOCK_SNMP_AGENT,
               PORT_SNMP_AGENT);
#endif
        break;
    ...
    *dataType = snmpData[id].dataType;
    ...
    case SNMPDTYPE_GAUGE: {
        ...
        */
        ...
        ptr_8 = ptr;
        // value = HTONL(snmpData[id].u.intval); // Endian convert when processing 32bit pointer
        // operation
...
void insertRespLen(int32_t reqStart, int32_t respStart, int32_t size) {
    ...
    if (request_msg.buffer[reqStart + 1] & 0x80) {
        ...
        while (lenLength--) {
            response_msg.buffer[indexStart + lenLength] = (uint8_t)((size & mask) >> shift);
            shift += 8;
            mask <<= shift;
        }
    } else {
...
int32_t snmp_sendTrap(uint8_t *managerIP, uint8_t *agentIP, int8_t *community,
                      dataEntryType enterprise_oid, uint32_t genericTrap, uint32_t specificTrap,
                      uint32_t va_count, ...) {
    uint32_t i;
