...
/**
 * @brief Sets a channel ON/OFF and mirrors the state to the display-board LED
 *   regardless of HAS_SCREEN. If HAS_SCREEN==1, it also updates the UI text.
 * @param channel Channel number (1..8)
 * @param on true = ON, false = OFF
 */
void PDU_Display_SetChannel(uint8_t channel, bool on) {
    if (channel < 1 || channel > 8)
        return;

    // Always drive the hardware (works in headless mode too)
    mcp_relay_write_pin(channel - 1, on);   // Relay board MCP23017 (I2C1)
    mcp_display_write_pin(channel - 1, on); // Display board MCP23017 (I2C0): OUT_1..OUT_8

    // If a screen is present, reflect the state textually as well
    if (HAS_SCREEN) {
        uint16_t y = BASE_Y + (channel - 1) * ROW_HEIGHT;
        // Clear previous text
        ...
        // Draw new state text
        ILI9488_DrawText(STATE_X, y, on ? "ON" : "OFF", on ? COLOR_GREEN : COLOR_RED);
    }
}
...
void PDU_Display_ToggleRelay(uint8_t channel) {
    ...
    PDU_Display_SetChannel(channel, relay_states[channel - 1]);
}
