...
void handle_settings_request(uint8_t sock) {
    ...
    DEBUG_PRINT("Network EEPROM: IP=%u.%u.%u.%u  GW=%u.%u.%u.%u\n", net.ip[0], net.ip[1], net.ip[2],
                net.ip[3], net.gw[0], net.gw[1], net.gw[2], net.gw[3]);

    // --- 2) Load prefs from EEPROM ----------------------------------------
    userPrefInfo pref;
    EEPROM_ReadUserPrefsWithChecksum(&pref);
    DEBUG_PRINT("Prefs EEPROM: device_name=\"%s\"  location=\"%s\"  unit=%u\n", pref.device_name,
    ...
    DEBUG_PRINT("Displaying temp = %.2f%s\n", temp_out, unit_suffix);
...
void handle_settings_post(uint8_t sock, char *body) {
    ...
    if (memcmp(&net, &backup_net, sizeof(net)) != 0) {
        ...
        DEBUG_PRINT("Network reconfigured\n");
    } else {
    ...
    if ((tmp = get_form_value(body, "device_name"))) {
        ...
        urldecode(tmp);
        INFO_PRINT("Decoded device_name=\"%s\"\n", tmp);
