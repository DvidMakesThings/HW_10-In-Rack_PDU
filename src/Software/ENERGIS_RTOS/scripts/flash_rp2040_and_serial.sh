...
mkdir -p "$LOG_DIR_ABS"
LOG_FILE="$LOG_DIR_ABS/energis_$(date +'%Y%m%d_%H%M%S').log"

python3 "$PY_UPLOADER_ABS" --p "$PORT" --b "$BAUD" --f "$FIRMWARE_ABS"

echo
echo "[*] Opening serial on $PORT @ $BAUD"
echo "    Log: $LOG_FILE"
echo "    Exit picocom with: Ctrl-A, then X"
echo

exec picocom -b "$BAUD" --imap lfcrlf --echo --logfile "$LOG_FILE" "$PORT"
